...
from discord_report import format_discord_report
from edsm_client import apply_edsm_system_data
from edsm_client import fetch_edsm_system as _fetch_edsm_system
...
def fetch_edsm_system(system: str) -> Dict[str, Any]:
    ...
    return _fetch_edsm_system(system, user_agent=config.user_agent)
...
# ---------------------------------------------------------------------------
# Journal handler (EDMC hook — pure logic lives in journal_handlers.py)
# ---------------------------------------------------------------------------
def journal_entry(
    cmdr: str,
    is_beta: bool,
...
def _copy_discord_report() -> None:
    ...
    try:
        if widget is not None:
            ...
            # Fallback when hooks run outside a full UI (tests / headless)
